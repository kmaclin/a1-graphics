...
class MovingSphere extends Sphere {
    ...
    getCenter(time: number): Vector {
        var sphere_center = new Vector(0.0, 1.0, -0.25);
        sphere_center.x = sphere_center.x + Math.sin(0)/2;
        var angle = (2 * Math.PI) * time;
        
        var sin = Math.sin(angle);
        sin = Math.pow(sin, 3);
        sphere_center.x = sphere_center.x + sin * 2;
...
class RayTracer {
    ...
    //    of frames
    render(scene, encoder: Whammy.Video, length: number, fps: number, grid: number,
        ...
        var getPoint = (x, y, camera) => {
            ...
            return Vector.norm(Vector.plus(Vector.times(-camera.dist, camera.forward), Vector.plus(Vector.times(recenterX(x), camera.right), Vector.times(recenterY(y), camera.up))));
        }
        ...
        var renderRow = () => {
            time = fcounter/(fps*length);
            for (var x = 0; x < screenWidth; x++) {
                var color = new Color(0, 0, 0);

                for (var p = 0; p < grid; p++) {
                    for (var q = 0; q < grid; q++) {
                        var newColor = this.traceRay({ start: scene.camera.pos, dir: getPoint(x + ((p + Math.random())/grid), y + ((q + Math.random())/grid), scene.camera) }, scene, 0, time + (Math.random()*(1/(fps*length))));

                       // var color = this.traceRay({ start: scene.camera.pos, dir: getPoint(x, y, scene.camera) }, scene, 0, time + (Math.random()*(1/(fps*length))));
                        color = Color.plus(color, newColor);
                    }
                 }
...
function defaultScene(): Scene {
    return {
        things: [new Plane(new Vector(0.0, 1.0, 0.0), 0.0, Surfaces.checkerboard),
                 ...
                 new Sphere(new Vector(-0.5, 0.4, 2.5), 0.4, Surfaces.shiny)],
        lights: [{ pos: new Vector(-2.0, 2.5, 0.0), color: new Color(0.49, 0.07, 0.07) },
                 { pos: new Vector(1.5, 2.5, 1.5), color: new Color(0.00, 0.34, 0.00) },
                 { pos: new Vector(1.5, 2.5, -1.5), color: new Color(0.07, 0.49, 0.071) },
                 { pos: new Vector(-1.5, 3.0, 0.0), color: new Color(0.47, 0.15, 0.04)},
                 { pos: new Vector(-1.0, 3.5, 0.0), color: new Color(0.21, 0.21, 0.35) }],
        camera: new Camera(new Vector(5.0, 4.0, 3.0), new Vector(-2.0, 0.5, 0.0), -2.0, 4, 3) //-1.0,0.5,0.0
    };
}
...
function exec() {
    ...
    // start the raytracer
    rayTracer.render(defaultScene(), encoder, length, fps, 4, ctx, 640, 480, 640, 480);
}
